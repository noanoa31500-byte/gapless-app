...
def detect_gml_ns(gml_path: str) -> str:
    """GMLファイルの名前空間を最初の数行から検出する。"""
    try:
        with open(gml_path, "rb") as f:
            head = f.read(1024).decode("utf-8", errors="replace")
        if "schemas.opengis.net/gml/3.2.1" in head:
            return "http://schemas.opengis.net/gml/3.2.1"
        if "www.opengis.net/gml/3.2" in head:
            return "http://www.opengis.net/gml/3.2"
        if 'www.opengis.net/gml"' in head or "www.opengis.net/gml'" in head:
...
def parse_gml_iterative(gml_path: str, gml_ns: str) -> dict:
    """
    ET.iterparse を使って大きなGMLファイルをメモリ効率よく読む。
    全要素を一度に展開せず、Surface/Curve/区域要素を逐次収集する。
    """
    ...
    area_tag_locals = {
        "PlanScale", "MaximumScale", "InundationTime",
        "A31", "FloodingArea", "floodingArea", "InundationArea", "HazardousZone",
    }
    ...
    try:
        ...
        for event, el in context:
            ...
            gid   = el.attrib.get(f"{{{gml_ns}}}id", "")

            if local == "Surface" and gid:
                surface_map[gid] = el
            elif local == "Curve" and gid:
                curve_map[gid] = el
            elif local in area_tag_locals:
                area_elements.append(el)
    except Exception as e:
        log(f"  iterparse エラー: {e}")

    return surface_map, curve_map, area_elements


def gml_to_geojson(gml_path: str) -> dict:
    log(f"  GML解析: {os.path.basename(gml_path)}")

    gml_ns = detect_gml_ns(gml_path)
    GML    = gml_ns
    XLINK  = "http://www.w3.org/1999/xlink"
    KSJ    = "http://nlftp.mlit.go.jp/ksj/schemas/ksj-app"

    try:
        surface_map, curve_map, area_elements = parse_gml_iterative(gml_path, GML)
    except Exception as e:
        log(f"  GML解析スキップ: {os.path.basename(gml_path)} ({e})")
        return {"type": "FeatureCollection", "features": []}

    if not surface_map and not area_elements:
        # フォールバック: 通常のパースを試みる（小さいファイル用）
        try:
...
def shp_to_geojson(shp_path: str) -> dict:
    """Shapefile を GeoJSON FeatureCollection に変換する。"""
    try:
        import shapefile
    except ImportError:
        log("  pyshp がインストールされていません: pip3 install pyshp --break-system-packages")
        return {"type": "FeatureCollection", "features": []}

    log(f"  SHP解析: {os.path.basename(shp_path)}")
    try:
        sf = shapefile.Reader(shp_path, encoding="cp932", encodingErrors="replace")
    except Exception:
        try:
    ...
    depth_field = None
    for f in fields:
        if f in ("A31_001", "waterDepth", "W_DEPTH", "DEPTH"):
            depth_field = f
            break
    ...
    for sr in sf.shapeRecords():
        ...
        record = sr.record
        depth_text = None
        if depth_field and depth_field in fields:
            val = record[fields.index(depth_field)]
            depth_text = str(val) if val else None
        risk_level = get_risk_from_depth(depth_text)
        if shape.shapeType in (5, 15, 25):
            ...
            if rings:
                features.append({
                    "type": "Feature",
                    "properties": {"risk_level": risk_level, "depth_rank": depth_text or "unknown"},
                    "geometry": {
...
def process_zip(zip_path: str, extract_dir: str, output_geojson: str) -> bool:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        gml_files = [n for n in all_names
                     if (n.endswith(".xml") or n.endswith(".gml"))
                     and not os.path.basename(n).startswith("KS-META")]
        shp_files = [n for n in all_names if n.endswith(".shp")]
        log(f"GMLファイル数: {len(gml_files)}  SHPファイル数: {len(shp_files)}")
    ...
    # SHP処理（GMLでデータが取れなかった場合）
    if not all_features and shp_files:
        log("  GMLからデータなし → Shapefile を使用します")
        for shp_name in shp_files:
            shp_path = os.path.join(extract_dir, shp_name)
            if os.path.exists(shp_path):
                result = shp_to_geojson(shp_path)
                all_features.extend(result.get("features", []))
...
def clip_features_to_bbox(features: list, bbox: tuple) -> list:
    ...
    for feat in features:
        ...
        if not raw_coords:
            continue
        # 座標を平坦化する（[[lng,lat],...] または [[[lng,lat],...]] など）
        ...
        coords = flatten_coords(raw_coords)
        if not coords:
            continue
        # 重心がbbox内かチェック
        lats = [float(c[1]) for c in coords if len(c) >= 2]
        lngs = [float(c[0]) for c in coords if len(c) >= 2]
...
def get_github_folder_files(folder: str) -> list:
    """GitHub の指定フォルダ内のファイル一覧を取得する。"""
    import http.client, ssl, urllib.parse as _up
    ...
    parsed = _up.urlparse(f"{GITHUB_API}/repos/{GITHUB_OWNER}/{GITHUB_REPO}/contents/{folder}?ref={GITHUB_BRANCH}")
    headers = {
    ...
    try:
        conn.request("GET", parsed.path + "?" + parsed.query, headers=headers)
...
def delete_github_file(repo_path: str, sha: str, message: str) -> bool:
    """GitHub のファイルを削除する。"""
    import http.client, ssl, urllib.parse as _up
    GITHUB_OWNER = "noanoa31500-byte"
    GITHUB_REPO  = "maps"
    ...
    def h(s): return s.encode("ascii", "ignore").decode("ascii")
    parsed = _up.urlparse(f"{GITHUB_API}/repos/{GITHUB_OWNER}/{GITHUB_REPO}/contents/{repo_path}")
    headers = {
        "Authorization": h(f"token {token}"),
        "Accept": "application/vnd.github+json",
...
def delete_existing_hazard_files(pref_key: str):
    """GitHubの県フォルダ内にある既存のハザードファイルを削除する。"""
    log(f"  既存ハザードファイルを削除中: {pref_key}/")
    files = get_github_folder_files(pref_key)
    deleted = 0
    for f in files:
        if f.get("type") == "file" and "_hazard" in f["name"]:
            ok = delete_github_file(
                f"{pref_key}/{f['name']}",
                f["sha"],
                f"Remove old hazard: {f['name']}"
            )
            if ok:
                log(f"  削除: {f['name']}")
                deleted += 1
            time.sleep(0.3)
    log(f"  削除完了: {deleted} ファイル")


def upload_hazard(produced_files: list, pref_key: str, dry_run: bool):
    """
    1. GitHub の県フォルダ内の既存ハザードファイルを削除
    2. 新しい市区町村単位の .gplh.gz をアップロード
    """
    from github_upload import upload_files

    if dry_run:
        log(f"[dry-run] アップロードをスキップ ({len(produced_files)} 市区町村)")
        return

    # 既存ファイルを削除
    delete_existing_hazard_files(pref_key)
...
def main():
    ...
    for pref_key in targets:
        result = process_prefecture(pref_key, args.dry_run)
        if result:
            succeeded.append(pref_key)
            if not args.geojson_only:
                upload_hazard(result, pref_key, args.dry_run)
        else:
