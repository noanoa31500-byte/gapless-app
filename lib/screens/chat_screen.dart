...
class _ChatScreenState extends State<ChatScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _addBotMessage(AppLocalizations.t('chat_prompt_main'));
    });
  }
  ...
  void _handleOptionSelected(String id) {
    // 0. Update UI
    final label = AppLocalizations.t('guide_$id');
    ...
    Future.delayed(const Duration(milliseconds: 1000), () {
      ...
      try {
        ...
        print('--- [DEBUG] AI System Prompt: \n$systemPrompt ---');
        // Personalization
        final profile = context.read<UserProfileProvider>().profile;
        String prefix = AppLocalizations.t('bot_prefix_normal');
        if (profile.name.isNotEmpty) {
           prefix = AppLocalizations.t('bot_prefix_name').replaceAll('@name', profile.name);
        }
        ...
        _addBotMessage(
          responseText,
          guideId: item.id,
          guideLabel: AppLocalizations.t('chat_btn_back'), 
        );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: SafeText(
          AppLocalizations.t('header_ai_guide'),
                         ...
                         Container(
                          ...
                          child: ElevatedButton.icon(
                            ...
                            label: SafeText(
                                ...
                                AppLocalizations.t('header_survival_guide'),
  ...
  Widget _buildOptionChips() {
    ...
    if (_menuState == 'main') {
      headerText = AppLocalizations.t('header_category_guide'); // Official 6
      options = SurvivalData.getOfficialGuides(region).map((e) => e.id).toList();
    } else {
      // 'more' -> AI Support (Expanded 10 items)
      headerText = AppLocalizations.t('header_category_ai');
        ...
        Wrap(
          ...
          children: [
            ...
            ActionChip(
              ...
              label: SafeText(
                _menuState == 'main' 
                    ? AppLocalizations.t('chat_btn_more') // "Other Topics" -> "AI Support"
                    : AppLocalizations.t('chat_btn_back'),
  ...
  String _buildFinalPrompt(String countryCode, bool isArrived) {
    ...
    String currentLang = AppLocalizations.lang; 
    
    if (countryCode == 'JP') {
      // --- 日本用プロンプト ---
      return """
あなたは災害支援アプリ「SafeJapan」の専用AIです。
...
"""
      ...
      if (!isArrived) {
        ...
        return """
あなたは災害支援アプリ「SafeJapan」の専用AIです。
現在、ユーザーはタイのパトゥムターニー県におり、避難所へ移動中です。

### 1. 現在のコンテキスト
- 地域: タイ・パトゥムターニー (Flood Risk)
...
ユーザーはタイ・パトゥムターニーの避難所に無事到着しました。これより生活支援を開始します。
