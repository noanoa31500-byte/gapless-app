      ...
      builder: (ctx) => Container(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(children: [
            ...
            const Expanded(child: Text('危険情報（未確認）',
              style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Color(0xFF2E7D32)))),
          ]),
          const SizedBox(height: 16),
          _buildInfoRow(Icons.map, '座標', '${spot.lat.toStringAsFixed(5)}, ${spot.lng.toStringAsFixed(5)}'),
          const SizedBox(height: 8),
          _buildInfoRow(Icons.schedule, '記録日時', spot.timestamp.toLocal().toString().substring(0, 16)),
          const SizedBox(height: 8),
          _buildInfoRow(Icons.people, '報告数', '${spot.reportCount}件'),
          const SizedBox(height: 8),
          _buildInfoRow(Icons.flag_outlined, 'ステータス', '未確認'),
        ]),
      ),
    );
  }
