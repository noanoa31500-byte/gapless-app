...
const _needItems = [
  ...
  _ChipItem(
      'Hearing Impairment', 'need_hearing', Icons.hearing_disabled_outlined),
  ...
  _ChipItem(
      'Service Animal', 'need_service_animal', Icons.support_agent_outlined),
...
const _conditionItems = [
  ...
  _ChipItem(
      'Mental Health', 'cond_mental_health', Icons.self_improvement_outlined),
  _ChipItem('Kidney Disease', 'cond_kidney_disease',
...
class _EmergencyCardPageState extends State<EmergencyCardPage> {
  ...
  Widget _buildIdentityCard(UserProfile p) {
    return _card(
      ...
      children: [
        _row(GapLessL10n.t('label_name'), p.name.isEmpty ? '-' : p.name,
            large: true),
        const SizedBox(height: 10),
        Row(children: [
          Expanded(
              child: _row(GapLessL10n.t('label_blood'),
                  p.bloodType.isEmpty ? '-' : p.bloodType)),
          Expanded(
              child: _row(GapLessL10n.t('label_nation'),
                  p.nationality.isEmpty ? '-' : p.nationality)),
        ]),
        ...
        if (p.emergencyContact.isNotEmpty || p.emergencyPhone.isNotEmpty) ...[
          ...
          _row(
              GapLessL10n.t('section_emergency_contact'),
    ...
    return _card(
      ...
      children: [
        ...
        if (p.conditions.isNotEmpty) ...[
          ...
          _chips(p.conditions, _conditionItems, const Color(0xFFFFF3E0),
              Color(0xFFE65100)),
        ],
      ],
    );
    ...
    return _card(
      ...
      children: [
        if (p.needs.isNotEmpty) ...[
          ...
          _chips(p.needs, _needItems, const Color(0xFFE3F2FD),
        ...
        if (p.languages.isNotEmpty) ...[
          ...
          _chips(p.languages, _languageItems, const Color(0xFFE8F5E9),
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          SafeText(title,
              style: safeStyle(size: 13, isBold: true, color: _kRed)),
        ]),
  ...
  Widget _label(String text) => Padding(
        ...
        child: SafeText(text,
            style: safeStyle(size: 12, isBold: true, color: Colors.grey[600]!)),
      );

  Widget _chips(
      List<String> selected, List<_ChipItem> items, Color bg, Color fg) {
    final labels = <Widget>[];
            ...
            builder: (ctx, provider, _) {
              ...
              return ListView(
                ...
                children: [
                  ...
                  _sectionHeader(GapLessL10n.t('section_basic_info'),
                      Icons.person_outline),
                  _field(GapLessL10n.t('label_name'), nameCtrl, (v) {
                    p.name = v;
                    provider.saveProfile(p);
                  }, hint: GapLessL10n.t('hint_name')),
                  const SizedBox(height: 12),
                  Row(children: [
                    Expanded(
                        child: _field(GapLessL10n.t('label_blood'), bloodCtrl,
                            (v) {
                      p.bloodType = v;
                      provider.saveProfile(p);
                    })),
                    ...
                    Expanded(
                        child:
                            _field(GapLessL10n.t('label_nation'), natCtrl, (v) {
                      p.nationality = v;
                      provider.saveProfile(p);
                    })),
                  ]),
                  const SizedBox(height: 12),
                  _field(GapLessL10n.t('label_dob'), dobCtrl, (v) {
                    p.birthDate = v;
                    provider.saveProfile(p);
                  }, hint: 'YYYY-MM-DD'),
                  const SizedBox(height: 20),

                  _sectionHeader(GapLessL10n.t('section_emergency_contact'),
                      Icons.phone_outlined),
                  _field(GapLessL10n.t('label_emergency_name'), ecCtrl, (v) {
                    p.emergencyContact = v;
                    provider.saveProfile(p);
                  }),
                  const SizedBox(height: 12),
                  _field(GapLessL10n.t('label_emergency_phone'), epCtrl, (v) {
                    p.emergencyPhone = v;
                    provider.saveProfile(p);
                  }, keyboardType: TextInputType.phone),
                  const SizedBox(height: 20),

                  _sectionHeader(GapLessL10n.t('section_medications'),
                      Icons.medication_outlined),
                  _field(GapLessL10n.t('label_medication_names'), medCtrl, (v) {
                    p.medications = v;
                    provider.saveProfile(p);
                  }, maxLines: 2),
                  const SizedBox(height: 24),

                  _sectionHeader(GapLessL10n.t('section_allergies'),
                  ...
                  _sectionHeader(GapLessL10n.t('section_conditions'),
                      Icons.local_hospital_outlined),
                  _chipSelector(
                      p.conditions, _conditionItems, const Color(0xFFE65100),
                      (id, sel) {
                  ...
                  _sectionHeader(
                      GapLessL10n.t('section_needs'), Icons.support_outlined),
                  _chipSelector(p.needs, _needItems, const Color(0xFF1565C0),
                      (id, sel) {
                  ...
                  _sectionHeader(GapLessL10n.t('section_languages'),
                      Icons.language_outlined),
                  _chipSelector(
                      p.languages, _languageItems, const Color(0xFF2E7D32),
                      (id, sel) {
  ...
  Widget _sectionHeader(String text, IconData icon) => Padding(
        ...
        child: Row(children: [
          ...
          const SizedBox(width: 6),
          SafeText(text,
              style: safeStyle(size: 14, isBold: true, color: _kRed)),
        ]),
      );

  Widget _field(
      String label, TextEditingController ctrl, Function(String) onChange,
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      TextField(
        ...
        decoration: InputDecoration(
          isDense: true,
          hintText: hint,
          hintStyle: GapLessL10n.safeStyle(
  ...
  Widget _chipSelector(List<String> selected, List<_ChipItem> items,
      Color activeColor, void Function(String id, bool selected) onChange) {
    return Wrap(
      spacing: 8,
      runSpacing: 8,
      children: items.map((item) {
        ...
        return FilterChip(
          avatar: Icon(item.icon,
              size: 14, color: isSel ? Colors.white : activeColor),
