...
class SafestRouteEngine {
  ...
  double calculateSurvivalCost(RoadEdge edge) {
    ...
    if (riskFactor == double.infinity) {
      if (kDebugMode) {
        debugPrint('🚫 絶対回避道路検出: ${edge.name ?? edge.id} '
    ...
    if (kDebugMode && riskFactor > 1.0) {
      debugPrint('⚠️ リスク道路: ${edge.name ?? edge.id} '
          '(type: ${edge.highwayType}, '
  ...
  List<String> findSafestPath(String startNodeId, String goalNodeId) {
    ...
    while (openSet.isNotEmpty) {
      ...
      if (currentNodeId == goalNodeId) {
        stopwatch.stop();
        if (kDebugMode) {
          debugPrint('✅ 最大安全ルート発見！');
          debugPrint('   探索ノード数: $nodesExplored');
          debugPrint('   探索時間: ${stopwatch.elapsedMilliseconds}ms');
        }
