...
enum GeoRegion {
  ...
  static GeoRegion fromCoordinates(double latitude, double longitude) {
    ...
    if (latitude >= 48 && latitude <= 60 && longitude >= -140 && longitude <= -115) {
...
class MagneticDeclinationConfig {
  ...
  static double getDeclination(double latitude, double longitude) {
    // テーブルキーを生成
    final key = '${latitude.toStringAsFixed(1)}_${longitude.toStringAsFixed(1)}';
...
class CompassCalibrator with ChangeNotifier {
  ...
  void setRegion(GeoRegion region) {
    ...
    if (kDebugMode) {
      print('🧭 CompassCalibrator: 地域変更 → ${region.nameJa} (偏角: ${region.declination}°)');
    }
  ...
  void setRegionFromCoordinates(double latitude, double longitude) {
    ...
    // より精密な偏角を取得
    _currentDeclination = MagneticDeclinationConfig.getDeclination(latitude, longitude);
    
    if (kDebugMode) {
      print('🧭 座標から偏角を取得: (${latitude.toStringAsFixed(4)}, ${longitude.toStringAsFixed(4)}) → ${_currentDeclination}°');
    }
  ...
  Map<String, dynamic> getDeclinationInfo() {
    ...
    final direction = dec < 0 ? '西偏 (West)' : dec > 0 ? '東偏 (East)' : 'なし';
