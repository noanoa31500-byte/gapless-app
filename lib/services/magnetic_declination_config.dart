...
class CompassCalibrator with ChangeNotifier {
  ...
  void setRegion(GeoRegion region) {
    ...
    if (kDebugMode) {
      debugPrint('🧭 CompassCalibrator: 地域変更 → ${region.nameJa} (偏角: ${region.declination}°)');
    }
  ...
  void setRegionFromCoordinates(double latitude, double longitude) {
    ...
    if (kDebugMode) {
      debugPrint('🧭 座標から偏角を取得: (${latitude.toStringAsFixed(4)}, ${longitude.toStringAsFixed(4)}) → ${_currentDeclination}°');
    }
