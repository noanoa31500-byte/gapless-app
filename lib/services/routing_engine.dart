...
class RoutingEngine {
  ...
  /// BLE スコアルックアップ: エッジ中心点 → スコア (0.0〜1.0)
  /// null なら BLE スコアを使わない
  final double Function(LatLng midpoint)? bleScoreLookup;
  ...
  double _calculateJapanWeight(RoadEdge edge) {
    ...
    final baseCost = edge.distance * widthMultiplier * timeDecay;
    return _applyBleScore(baseCost, edge.centerPoint);
  }
  ...
  double _calculateThailandWeight(RoadEdge edge) {
    ...
    return _applyBleScore(edge.distance * penaltyMultiplier, edge.centerPoint);
  }
  ...
  /// BLE スコアに基づいてコストを調整する
  ///   score=0.0 (通行不可) → infinity
  ///   score=0.3 (要注意)   → コスト×3
  ///   score=1.0 (安全)     → コスト×0.8
  ///   score=0.5 (中立)     → コスト変更なし
