...
class SafeNavigationService {
  ...
  Future<void> initialize() async {
    ...
    _isDataLoaded = true;
    debugPrint('✅ SafeNavigationService initialized');
  }
  ...
  void setHazardPolygons(List<List<LatLng>> polygons) {
    ...
    debugPrint('⚠️ SafeNav: Updated ${_externalHazardPolygons.length} external hazard polygons');
  }
      ...
      if (data['type'] == 'point_hazard') {
        ...
        _floodRiskPoints = points
            ...
            .where((p) {
              ...
              final depth = (p['pred_depth'] as num?)?.toDouble() ?? 0.0;
              return t.contains('flood') || depth > 0.3;
            })
            .map((p) => LatLng(
                  (p['lat'] as num).toDouble(),
                  (p['lng'] as num? ?? p['lon'] as num? ?? 0).toDouble(),
                ))
            .toList();
      }
      debugPrint('🌊 Loaded ${_floodRiskPoints.length} flood risk points');
    } catch (e) {
  ...
  Future<void> _loadPowerData() async {
    try {
      ...
      debugPrint('⚡ Loaded ${_powerLinePolylines.length} power risk lines');
    } catch (e) {
      debugPrint('❌ Error loading power data: $e');
    }
  }
  ...
  List<LatLng> findSafeRoute(LatLng start, LatLng end) {
    ...
    while (openList.isNotEmpty) {
      ...
      if (iterations > maxIterations) {
        debugPrint('⚠️ Pathfinding output limited by max iterations');
    ...
    debugPrint('⚠️ SafeNav: No safe route found after $iterations iterations. Returning direct path.');
    return [start, end];
  }
  ...
  void debugPrintRoute(List<LatLng> route) {
    ...
    debugPrint('🧩 --- Safe Route Debug ---');
    debugPrint('Total Points: ${route.length}');
    for (int i = 0; i < route.length; i++) {
        debugPrint('WP[$i]: ${route[i].latitude}, ${route[i].longitude}');
    }
    debugPrint('-------------------------');
  }
