...
class OfflineRiskScanner {
  ...
  Future<void> loadData() async {
    ...
    try {
      ...
      if (kDebugMode) {
        print('⚡ OfflineRiskScanner: データロード完了');
        print('   - 送電線セグメント: ${_powerLines.length}');
        print('   - 浸水予測ポイント: ${_floodPoints.length}');
      }
    } catch (e) {
      _loadError = e.toString();
      if (kDebugMode) {
        print('❌ OfflineRiskScanner: データロードエラー: $e');
      }
    } finally {
      _isLoading = false;
    }
  ...
  Future<void> _loadPowerRiskData() async {
    try {
      final jsonString = await rootBundle.loadString('assets/data/power_risk_th.geojson');
      ...
      for (final feature in features) {
        ...
        if (geometry['type'] == 'LineString') {
          ...
          final voltage = _parseVoltage(properties['voltage']);
          final name = properties['name'] as String? ?? 'Unknown';
          
          // 線分を個別のセグメントに分割
          for (int i = 0; i < coordinates.length - 1; i++) {
            final start = coordinates[i] as List<dynamic>;
            final end = coordinates[i + 1] as List<dynamic>;
  ...
  Future<void> _loadFloodPredictionData() async {
    try {
      final jsonString = await rootBundle.loadString('assets/data/satun_flood_prediction.json');
      ...
      for (final point in data) {
        final floodPoint = _FloodPoint(
          ...
          riskScore: (point['risk_score'] as num?)?.toInt() ?? 0,
          displayType: point['display_type'] as String? ?? 'Unknown',
        );
      ...
      if (kDebugMode) {
        print('⚠️ 浸水予測データのロードをスキップ: $e');
      }
    }
  }

  /// 電圧をパース
  int _parseVoltage(dynamic voltage) {
    if (voltage == null) return 0;
    if (voltage is int) return voltage;
    if (voltage is String) {
      return int.tryParse(voltage.replaceAll(RegExp(r'[^0-9]'), '')) ?? 0;
    }
    return 0;
  }
  ...
  /// デバッグ出力
  void debugPrint(RiskScanResult result) {
