...
class SurvivalDataTH {
  ...
  static const List<SurvivalGuideItem> officialGuides = [
    SurvivalGuideItem(
      ...
      title: {'en': 'Electrical Safety', 'ja': '電気・感電への注意', 'th': 'ระวังไฟดูด (Electrocution)'},
      action: {
        'en': 'Cut power if flooded. Stay away from wet poles.',
        'ja': '浸水時はブレーカーを切って電気を遮断し、濡れた電柱や電線には絶対に近づかないでください。',
        'th': 'ตัดคัทเอาท์ทันทีหากน้ำท่วมบ้าน อย่าเข้าใกล้เสาไฟที่แช่น้ำ อันตรายถึงชีวิต',
      },
    ...
    SurvivalGuideItem(
      ...
      title: {'en': 'Flood Diseases', 'ja': '水害時の感染症対策', 'th': 'โรคระบาด (ฉี่หนู/ไข้เลือดออก)'},
      action: {
        'en': 'Do not wade in water (Leptospirosis). Use net (Dengue).',
        'ja': '汚水の中を素足で歩かない（レプトスピラ症対策）。蚊に刺されないよう蚊帳などを使う（デング熱対策）。',
        'th': 'ห้ามลุยน้ำเปล่าเท้า (โรคฉี่หนู) นอนกางมุ้งระวังยุงลาย (ไข้เลือดออก)',
      },
    ...
    SurvivalGuideItem(
      ...
      title: {'en': 'Poisonous Animals', 'ja': '毒を持つ生物への注意', 'th': 'สัตว์มีพิษ (งู/ตะขาบ)'},
      action: {
    ...
    SurvivalGuideItem(
      ...
      title: {'en': 'Clean Water/Food', 'ja': '水・食料衛生', 'th': 'กินร้อน ช้อนกลาง (Food Safety)'},
      action: {
        'en': 'Drink bottled water ONLY. Eat cooked food.',
        'ja': 'ボトル水のみ飲む。加熱した食事のみ摂る。',
        'th': 'ดื่มน้ำบรรจุขวดเท่านั้น กินอาหารปรุงสุกใหม่ๆ ระวังโรคท้องร่วงรุนแรง',
      },
    ...
    SurvivalGuideItem(
      ...
      title: {'en': 'Emergency #', 'ja': '緊急連絡先', 'th': 'เบอร์ฉุกเฉิน (1669/191)'},
      action: {
  ...
  static const List<SurvivalGuideItem> aiSupportGuides = [
     SurvivalGuideItem(
      id: 'drowning',
      icon: Icons.pool,
      title: {'en': 'Drowning Prevention', 'ja': '水の事故防止', 'th': 'ป้องกันจมน้ำ'},
      action: {
        'en': 'Do not let children play in flood water. It is deeper and faster than it looks.',
        'ja': '子供を水辺で遊ばせないでください。見た目より深く、流れが速いです。',
        'th': 'ห้ามเด็กเล่นน้ำท่วมเด็ดขาด น้ำอาจลึกและไหลเชี่ยวกว่าที่คิด (สาเหตุการตายอันดับ 1)',
      },
      source: 'MoPH (DDC)',
    ),
     SurvivalGuideItem(
    ...
    SurvivalGuideItem(
      ...
      title: {'en': 'Fungal Infection (Feet)', 'ja': '足の皮膚病（水虫など）', 'th': 'โรคน้ำกัดเท้า'},
      action: {
        'en': 'Keep feet dry. Apply fungicide cream if available. Do not stay in wet socks.',
    ...
    SurvivalGuideItem(
      id: 'toilet_th',
      icon: Icons.wc,
      title: {'en': 'Sanitation / Toilet', 'ja': 'トイレと衛生管理', 'th': 'สุขา / การขับถ่าย'},
      action: {
        'en': 'Use floating toilets or "black bags". Add lime/ash to reduce smell.',
        'ja': '浮きトイレや指定のゴミ袋を使用してください。臭い対策には石灰や灰が有効です。',
        'th': 'ใช้ส้วมลอยน้ำหรือถ่ายใส่ถุงดำ (ถุงยังชีพ) โรยปูนขาว/ขี้เたとぶんมัดปากถุงให้แน่น',
      },
    ...
    SurvivalGuideItem(
      ...
      action: {
        'en': 'Seal ID, Blue Book (Tabien Baan), and land deeds in plastic bags.',
        'ja': 'IDカード、タビアンバーン（青い本）をビニール袋で密封。',
        'th': 'เก็บถุงพลาสติกใส่บัตรประชาชน ทะเบียนบ้าน และโฉนดที่ดิน กันน้ำให้ดีที่สุด',
      },
      source: 'DPM',
    ),
    SurvivalGuideItem(
      ...
      action: {
        'en': 'Separate hazardous waste (lamps, spray cans). Do not throw trash in water.',
        'ja': '危険物（蛍光灯、スプレー）は分ける。水にゴミを捨てない。',
        'th': 'แยกขยะอันตราย (หลอดไฟ, กระป๋องสเปรย์) ห้ามทิ้งขยะลงน้ำ ป้องกันโรค',
      },
    ...
    SurvivalGuideItem(
      ...
      action: {
        'en': 'Keep medicines high and dry. Continue taking meds as prescribed.',
    ...
    SurvivalGuideItem(
      ...
      action: {
        ...
        'th': 'ช่วยเหลือเพื่อนบ้าน ผู้สูงอายุ แบ่งปันอาหารและน้ำดื่ม (คนไทยไม่ทิ้งกัน)',
      },
